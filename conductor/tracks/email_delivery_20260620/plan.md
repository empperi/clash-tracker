...
- [ ] Task: Tests + implement pure predicates in `@clash-tracker/core`: `isValidOtpFormat(code)`
  (exactly 6 digits), `isOtpExpired(now, expiresAt)`, and `hasExceededOtpAttempts(attempts, max)`.
